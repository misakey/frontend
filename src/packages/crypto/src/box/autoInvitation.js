...
export async function tryAddingAutoInvite({ event, boxSecretKey, boxKeyShare }) {
  if (
    event.type !== ACCESS_ADD
    || event.content.restrictionType !== RESTRICTION_TYPES.IDENTIFIER
  ) {
  ...
  result.forServerNoStore = Object.fromEntries(
  ...
  result.content.autoInvite = true;
