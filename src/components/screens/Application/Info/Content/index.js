...
import ApplicationInfoContentDomains from 'components/screens/Application/Info/Content/Domains';
...
const getPrivacyShieldCustomizer = () => {
  ...
  const format = (value, key, t) => (
    <Typography variant="body2">
      <MUILink
        href={t('screens:application.info.privacyShieldLink')}
        target="_blank"
...
const getMissingLinkCustomizer = (t, handleClick, userManager, isAuthenticated, mainDomain) => {
  ...
  const format = () => {
    if (isAuthenticated) {
      return (
        <MUILink onClick={handleClick} className="lightLink" component="button">
...
const ApplicationInfoContent = ({
  ...
  const missingLinkCustomizer = React.useMemo(
    ...
    [t, onContributionLinkClick, userManager, isAuthenticated],
  );
